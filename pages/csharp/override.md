...
- `virtual`は基底classでメソッドを「派生classで上書き可能」と宣言するキーワード
- `override`は派生classでその実装を上書きするキーワード
...
- 基底classのメソッドに`virtual`を付けると、実行時にインスタンスの実際の型のメソッドが呼ばれる
- 派生classで`override`すると、基底class経由で呼んでも派生class側の実装が実行される（動的ディスパッチ）
...
- **コンストラクタからvirtualメソッドを呼ぶ**
  - 派生classの初期化が終わる前にoverride先が実行され、未初期化フィールドを参照するバグになる
