...
- 小さな値をクラスで表すと、都度ヒープ確保が発生しGC負荷が増える場合がある
...
- クラスと違い、structには継承ができない（インターフェースの実装は可能）
...
- 使い分けの判断基準
  ...
  - 参照渡しで共有したい、または大きく可変なデータはクラスを使う
...
- structをインターフェース型の変数へ代入・キャストするとボックス化が発生し、ヒープ確保とGC負荷が生じる
