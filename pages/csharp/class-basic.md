...
- classはデータとふるまい（フィールド・プロパティ・メソッド）をひとまとめにした参照型の設計図
- オブジェクト指向設計の基本単位で、業務ロジックの大半はclassとして書かれる
- 読了後、classの基本構文とインスタンス化の流れを理解できる

## なぜ必要なのか

- 関連するデータと処理をまとめないと、引数だらけの関数が散乱し保守性が下がる
- classを使うと状態と操作を1つの型に閉じ込め、名前空間的にも整理できる
...
- **参照の共有を意識しない代入**
  - class変数の代入はインスタンスのコピーではなく参照のコピー
...
- [ ] classが参照型であることを説明できる
- [ ] インスタンスの代入が参照のコピーであることを説明できる
- [ ] コンストラクタとプロパティを使った基本的なclassを書ける
